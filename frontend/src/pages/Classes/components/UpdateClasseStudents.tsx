...
import styles from '../styles/UpdateClasseStudents.module.css';
...
import { apiUpdateClasseStudents } from '~api/classe';
...
type UpdateClasseStudentsProps = {
    classeDetail: ClasseDetail;
    onMutateSuccess: () => void;
    setShowPopUp: React.Dispatch<React.SetStateAction<boolean>>;
};

export default function UpdateClasseStudents({
    classeDetail,
    onMutateSuccess,
    setShowPopUp
}: UpdateClasseStudentsProps) {
    const language = useLanguage('component.update_classe_students');
    const [queryUser, setQueryUser] = useState('');
    const debounceQueryUser = useDebounce(queryUser, AUTO_COMPLETE_DEBOUNCE);
    const [students, setStudents] = useState(classeDetail.enrollments.map(item => item.user));
    const handleClosePopUp = () => {
        setShowPopUp(false);
    };
    const handleUpdateClasseStudents = async () => {
        const studentIds = students.map(student => student.id);
        await apiUpdateClasseStudents(studentIds, classeDetail.id);
    };
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: handleUpdateClasseStudents,
        onSuccess: onMutateSuccess
    });
    return (
        <div className={
            css(
                styles.updateClasseStudentsContainer,
            )
        }>
            {
            ...
            <div className={
                css(
                    styles.updateClasseStudentsForm,
                )
            }>
                ...
                <div className={styles.formContent}>
                    ...
                    <div className={styles.wrapDataContainer}>
                        <div className={styles.dataContainer}>
                            ...
                            <ul className={styles.allStudentContainer}>
                                {userQueryData.data ?
                                    userQueryData.data
