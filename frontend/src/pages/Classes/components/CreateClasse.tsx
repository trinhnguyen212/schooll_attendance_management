...
import { apiCreateClasse } from '~api/classe';
import { apiAutoCompleteCourse } from '~api/course';
...
export default function CreateClasse({
    ...
    const language = useLanguage('component.create_classe');
    const [queryUser, setQueryUser] = useState('');
    const [queryCourse, setQueryCourse] = useState('');
    const [shortcode, setShortcode] = useState('');
    const debounceQueryUser = useDebounce(queryUser, AUTO_COMPLETE_DEBOUNCE);
    const debounceQueryCourse = useDebounce(queryCourse, AUTO_COMPLETE_DEBOUNCE);
    ...
    const courseQueryData = useQuery({
        queryKey: [QUERY_KEYS.AUTO_COMPLETE_COURSE, { search: debounceQueryCourse }],
        queryFn: () => apiAutoCompleteCourse(debounceQueryCourse),
        enabled: debounceQueryCourse ? true : false
    });
    const handleCreateClasse = async (e: SyntheticEvent<HTMLFormElement, SubmitEvent>) => {
        e.preventDefault();
        ...
        await apiCreateClasse(formData);
        if (submitter.name === 'save') handleClosePopUp();
        else form.reset();
    };
    const { mutate, isPending } = useMutation({
        mutationFn: handleCreateClasse,
                ...
                <div className={styles.formContent}>
                    <form onSubmit={(e: SyntheticEvent<HTMLFormElement, SubmitEvent>) => {
                        ...
                        <div className={styles.groupInputs}>
                            ...
                            <div className={styles.wrapItem}>
                                <label className={appStyles.required} htmlFor='course_id'>{language?.course}</label>
                                <CustomDataList
                                    name='course_id'
                                    onInput={e => { setQueryCourse(e.currentTarget.value); }}
                                    options={courseQueryData.data ? courseQueryData.data.map(item => {
                                        return {
                                            label: item.name,
                                            value: String(item.id)
                                        };
