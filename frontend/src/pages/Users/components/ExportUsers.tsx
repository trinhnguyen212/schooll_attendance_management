...
export default function ExportUsers({
    ...
    const handleExportUsers = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (applyFilterRef.current?.checked) {
          ...
          //  query.schoolClassId = searchParams.get(schoolClassFilterKey) || undefined;
            query.schoolOfThoughtId = searchParams.get(schoolOfThoughtFilterKey) || undefined;
            query.branchId = searchParams.get(branchFilterKey) || undefined;
  ...
  {role === 'student'
    ? language?.exportStudents
    : role === 'teacher'
    ? language?.exportTeachers
    : role === 'parent'
    ? language?.exportParents
    : role === 'admin'
    ? language?.exportAdmins
    : language?.exportUsers}
</h2>
